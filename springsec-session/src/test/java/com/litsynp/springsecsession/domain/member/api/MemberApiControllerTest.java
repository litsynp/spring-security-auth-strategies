...
import static com.litsynp.springsecsession.config.SpringSecurityWebAuxTestConfig.getBasicMember;
...
import com.litsynp.springsecsession.config.SpringSecurityWebAuxTestConfig;
...
import com.litsynp.springsecsession.global.config.SecurityConfig;
...
class MemberApiControllerTest {
    ...
    void deleteMember_ok() throws Exception {
        // given
        given(memberService.findById(getBasicMember().getId()))
                .willReturn(getBasicMember());

        // when & then
        mockMvc.perform(delete("/v1/members/{id}", getBasicMember().getId())
                        .accept(MediaType.APPLICATION_JSON)
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
